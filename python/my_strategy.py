...
class MyStrategy:
    ...
    def first_time_preparation(self, game):
        ...
        for planet in resource_planets:
            ...
            planets_workers_count[planet.id] = 0 # обнуляем количесвто рабочих в начале игры (чтобы не отправляли нескоько раз рабочих)
    ...
    def init_values(self, game):
        ...
        for planet in game.planets:
            if planet.harvestable_resource is not None and planet.harvestable_resource is not Resource.STONE:  # закидываем ресурс (НЕ камень)
    ...
    def action_generation(self, game):
        ...
        for worker in my_workers:
            for planet in resource_planets:
                ...
                if planets_workers_count[planet.id] < 50 and Resource.STONE in game.planets[worker[0]].resources.keys(): # есть ли камни там, есть ли рабочие (< 50)
                    ...
                    if game.planets[worker[0]].resources[Resource.STONE] - taken_stone[worker[0]] >= 50 \
                        ...
                        moves.append(MoveAction(worker[0], planet.id, 50, Resource.STONE))
        ...
        for planet in resource_planets:
            ...
            if have_workers and Resource.STONE in planet.resources.keys() and planet.resources[
                ...
                builds.append(BuildingAction(planet.id, types_of_buildings[planet.harvestable_resource])) # строим здание
